...
class WorkflowOrchestratorAgent(BaseAgent):
    ...
    def run_complete_workflow(
        ...
        def _workflow_operation():
            ...
            try:
                ...
                repo_path = self._setup_repository(repo_url, workdir, session)
                if not repo_path:
    ...
    def _initialize_workflow_session(
        ...
        metadata = self.github_agent.generate_pr_metadata(user_prompt)
        if metadata:
            session["branch_slug"] = metadata.get("slug")
            session["commit_message"] = metadata.get("commit_message")
            session["pr_title"] = metadata.get("title")
        else:
            slug = re.sub(r"[^a-zA-Z0-9\- ]", "", user_prompt).strip().lower()
            slug = re.sub(r"\s+", "-", slug) or "update"
            session["branch_slug"] = slug
            fallback_title = user_prompt.strip() or "update"
            session["pr_title"] = f":robot: [orion] {fallback_title}"
            session["commit_message"] = session["pr_title"]
    ...
    def _setup_repository(
        self, repo_url: str, workdir: Optional[str], session: Dict
    ) -> Optional[str]:
        """Setup the repository for the workflow."""
        if workdir is None:
            workdir = "/Users/ishandutta/Documents/code"  # Use default from main.py

        # Extract repo name and prepare branch name
        repo_name = repo_url.split("/")[-1].replace(".git", "")
        branch_slug = session.get("branch_slug", "update")
        base_branch_name = f"orion/{branch_slug}"
    ...
    def _commit_and_push(
        ...
        # Commit changes
        commit_message = session.get(
            "commit_message", f":robot: [orion] {user_prompt}"
        )
        ...
        if create_pr:
            ...
            if current_branch:
                ...
                if push_success:
                    # Create pull request
                    pr_title = session.get(
                        "pr_title", f":robot: [orion] {user_prompt}"
                    )
                    pr_body = f"This PR contains AI-generated changes for: {user_prompt}\n\nGenerated by Orion AI Agent"
